...
class Topico {
  static async findById(id) {
    const { rows } = await pool.query("SELECT * FROM topicos WHERE id = $1", [
      id,
    ]);
    return rows[0];
  }
  ...
  static async incrementViews(id) {
    await pool.query("UPDATE topicos SET views = views + 1 WHERE id = $1", [
      id,
    ]);
  }
  ...
  static async updateTopicoRating(topicoId, avgRating) {
    await pool.query("UPDATE topicos SET rating = $1 WHERE id = $2", [
      avgRating,
      topicoId,
    ]);
  }
      ...
      if (tags && tags.length > 0) {
        for (const tagName of tags) {
          ...
          let tagResult = await pool.query(
            "SELECT id FROM tags WHERE nome = $1",
            [tagName]
          );
          let tagId;
          if (tagResult.rows.length === 0) {
            tagResult = await pool.query(
              "INSERT INTO tags (nome) VALUES ($1) RETURNING id",
              [tagName]
            );
  ...
  static async update(
    id,
    { titulo, descricao, categoria_id, tags, ativo, status }
  ) {
    try {
      const updates = [];
      const values = [];
