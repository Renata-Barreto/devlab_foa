...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (extname && mimetype) {
      ...
      console.log(
        `Arquivo rejeitado: ${file.originalname} (apenas JPEG/PNG são permitidos)`
      );
...
const userController = {
  create: async (req, res) => {
    try {
      const { name, email, password, end_usr, cid_usr, est_usr, dtn_usr } =
        req.body;
      if (!name || !email || !password) {
        console.log(
          "Erro: Campos obrigatórios (name, email, password) não fornecidos"
        );
        return res
          .status(400)
      ...
      upload(req, res, async (err) => {
        try {
          ...
          if (bio && bio.length > 1000) {
            ...
            return res
              .status(400)
              .json({ message: "Bio não pode exceder 1000 caracteres" });
          }
          ...
          if (!user) {
            console.log(
              `Erro: Usuário ID ${userId} não encontrado após atualização`
            );
          ...
          console.error("Erro interno no update:", innerErr);
          return res
            .status(500)
            .json({
  ...
  delete1: async (req, res) => {
    try {
      ...
      if (!user) {
        console.log(
          `Erro: Usuário ID ${userId} não encontrado para deleção permanente`
        );
        return res.status(404).json({ message: "Usuário não encontrado" });
      }
