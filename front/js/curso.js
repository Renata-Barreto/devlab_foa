      ...
      <div class="module-inner">
        <div class="module-head">
          <div style="display:flex;gap:12px;align-items:center">
            <div class="icone"><i>${mod.concluido ? "✔" : (mod.aberto ? "○" : "<i class='fa-solid fa-lock'></i>")}</i></div>
            <div>
              <h3>${mod.nome}</h3>
      ...
      la.innerHTML = `
        ...
        <div style="flex:1">
          <div style="display:flex;gap:8px;align-items:center">
            ...
            ${a.status === "concluida" ? '<span class="badge-done" style="margin-left:8px">Concluída</span>' : ''}
          </div>
          <div style="font-size:13px;color:#777">${a.subtitulo || ''}</div>
        </div>
        <div style="margin-left:8px">
          ${a.liberada ? `<button class="btn-small open-aula" data-id="${a.id}">Abrir</button>` : `<span class="tooltip" data-tip="Conclua a aula anterior para liberar">🔒</span>`}
        </div>
      `;
